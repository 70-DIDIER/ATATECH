...
Le serveur affiche l'adresse à utiliser, par exemple `http://172.20.10.2:5055`.

Vérifiez que vous le joignez — **cette route ne demande pas de clé**, c'est
exprès, c'est votre test réseau :

```bash
curl http://192.168.1.24:5000/api/v1/ping
```
```json
...
```bash
curl -X POST http://192.168.1.24:5000/api/v1/demarches/session \
...
```bash
curl -X POST http://192.168.1.24:5000/api/v1/demarches/message \
  -H "X-Api-Cle: $CLE" -H "Content-Type: application/json" \
...
```bash
curl -X POST http://192.168.1.24:5000/api/v1/demarches/photo \
  -H "X-Api-Cle: $CLE" \
